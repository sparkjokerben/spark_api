...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	builder := r.client.APIKey.Create().
		...
		SetQuotaStickyMode(key.QuotaStickyMode)
	...
	m, err := r.activeQuery().
		...
		WithGroup(func(q *dbent.GroupQuery) {
			q.Select(
				...
				group.FieldQuotaStickyDefaultEnabled,
				group.FieldQuotaStickyUserOverrideAllowed,
				group.FieldSessionModelStabilityEnabled,
				group.FieldUnifiedRetryBudgetEnabled,
			)
		}).
		Only(ctx)
	if err != nil {
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	out := &service.APIKey{
		...
		QuotaStickyMode: m.QuotaStickyMode,
	}
	if m.Edges.User != nil {
...
func groupEntityToService(g *dbent.Group) *service.Group {
	...
	return &service.Group{
		...
		RPMLimit:                        g.RpmLimit,
		QuotaStickyDefaultEnabled:       g.QuotaStickyDefaultEnabled,
		QuotaStickyUserOverrideAllowed:  g.QuotaStickyUserOverrideAllowed,
		SessionModelStabilityEnabled:    g.SessionModelStabilityEnabled,
		UnifiedRetryBudgetEnabled:       g.UnifiedRetryBudgetEnabled,
